...
const Button = ({ children, green, outline, small, full }) => (
  <div style={{
    padding: small ? "4px 10px" : "6px 14px",
    background: green ? COLORS.whatsapp : outline ? "transparent" : COLORS.accent,
...
const LatestPieces = ({ isMobile }) => {
  ...
  const items = ["Espresso Cup — Ember", "Dinner Plate — Ocean", "Sake Set — Ash", "Bowl — Terracotta", "Mug — Midnight", "Vase — Sand"];
  return (
        ...
        {items.slice(0, isMobile ? 4 : 6).map((item, i) => (
          <div key={i} style={{ background: "#fff", borderRadius: 4, overflow: "hidden" }}>
            ...
            <div style={{ padding: "6px 8px" }}>
              <div style={{ fontSize: 7.5, fontWeight: 600, color: COLORS.dark, lineHeight: 1.3 }}>{item}</div>
              <div style={{ fontSize: 7, color: COLORS.accent, fontWeight: 600, marginTop: 2 }}>LKR 2,500</div>
  ...
  return (
    <Section>
      ...
      <div style={{ textAlign: "center", marginTop: 8, display: "flex", justifyContent: "center", gap: 8 }}>
        <Button outline small>Follow on Instagram →</Button>
      </div>
    </Section>
  );
};
...
const ReservationFlow = ({ isMobile }) => (
  <Section title="How to Reserve">
    <div style={{
      ...
      {[
        { num: "1", title: "Browse & Choose", desc: "Find a piece you love from our collection" },
        { num: "2", title: "Reserve on WhatsApp", desc: "Tap reserve — we'll confirm availability" },
        { num: "3", title: "Bank Transfer", desc: "Send payment to our bank details" },
        { num: "4", title: "Collect or Deliver", desc: "Pick up from studio or we'll ship" },
      ].map((step, i) => (
    ...
    <div style={{
      ...
      <div style={{ flex: isMobile ? "none" : 1, padding: isMobile ? "8px 12px" : "12px 16px 12px 0" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
          <Button green full>💬 Reserve on WhatsApp</Button>
        </div>
        <div style={{ textAlign: "center", marginTop: 4 }}>
          <span style={{ fontSize: 7, color: COLORS.muted, textDecoration: "underline" }}>Or fill out a reservation form</span>
        </div>
      </div>
    </div>
  </div>
  ...
  <div style={{
    ...
    <div style={{ display: "flex", gap: isMobile ? 16 : 32 }}>
      ...
      <div>
        ...
        <div style={{ lineHeight: 1.8 }}>WhatsApp<br />Instagram<br />Email</div>
      </div>
    </div>
    ...
    <div style={{
      ...
      <div style={{
        ...
        {[
          ...
          { badge: "RESERVATION", title: "WhatsApp-First Flow", desc: "Primary CTA is WhatsApp with pre-filled message (piece name + code). Fallback reservation form for international buyers." },
